...
template defineArithAsgnVectorScalarOp(op: untyped) =
  proc op*[I, T](a: var VecBase[I, T], s: T) {.nimslmagic.} =
    forEachComponentI(I):
      `op`(a[i], s)

defineArithAsgnVectorScalarOp(`+=`)
defineArithAsgnVectorScalarOp(`-=`)
defineArithAsgnVectorScalarOp(`*=`)
defineArithAsgnVectorScalarOp(`/=`)
...
template defineArithAsgnVectorVectorOp(op: untyped) =
  proc op*[I, T](a: var VecBase[I, T], b: VecBase[I, T]) {.nimslmagic.} =
    forEachComponentI(I):
      `op`(a[i], b[i])

defineArithAsgnVectorVectorOp(`+=`)
defineArithAsgnVectorVectorOp(`-=`)
defineArithAsgnVectorVectorOp(`*=`)
defineArithAsgnVectorVectorOp(`/=`)
...
proc normalize*[I, T](v: VecBase[I, T]): VecBase[I, T] {.nimslmagic.} =
  let len = length(v)
  for i in 0 ..< I:
    result[i] = v[i] / len
